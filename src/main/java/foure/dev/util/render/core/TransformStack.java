...
public final class TransformStack {
   ...
   public void pushRotation(float degrees) {
      float rad = (float)Math.toRadians((double)degrees);
      float c = (float)Math.cos((double)rad);
      float s = (float)Math.sin((double)rad);
      float[] r = new float[]{c, -s, 0.0F, s, c, 0.0F, 0.0F, 0.0F, 1.0F};
      float[] top = (float[])this.stack.peek();
   ...
   public void replaceTop(float[] matrix) {
      if (matrix == null) {
         ...
         if (this.stack.isEmpty()) {
            ...
            copy = Arrays.copyOf(matrix, matrix.length);
            this.stack.pop();
   ...
   private static float[] mul(float[] a, float[] b) {
      float[] r = new float[]{a[0] * b[0] + a[1] * b[3] + a[2] * b[6], a[0] * b[1] + a[1] * b[4] + a[2] * b[7], a[0] * b[2] + a[1] * b[5] + a[2] * b[8], a[3] * b[0] + a[4] * b[3] + a[5] * b[6], a[3] * b[1] + a[4] * b[4] + a[5] * b[7], a[3] * b[2] + a[4] * b[5] + a[5] * b[8], a[6] * b[0] + a[7] * b[3] + a[8] * b[6], a[6] * b[1] + a[7] * b[4] + a[8] * b[7], a[6] * b[2] + a[7] * b[5] + a[8] * b[8]};
      return r;
   }
}
