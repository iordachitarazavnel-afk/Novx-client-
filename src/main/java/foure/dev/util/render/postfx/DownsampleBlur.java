...
public final class DownsampleBlur {
   ...
   private int quadVao;
   private int quadVbo;
   private final DownsampleBlur.LevelTarget[] pyramid;
   private final DownsampleBlur.LevelTarget fullResolutionTarget;
   private final DownsampleBlur.LevelTarget smallTempTarget;
   ...
   public DownsampleBlur(int intermediateInternalFormat, int intermediatePixelType) {
      this.pyramid = new DownsampleBlur.LevelTarget[6];
      this.fullResolutionTarget = new DownsampleBlur.LevelTarget();
      this.smallTempTarget = new DownsampleBlur.LevelTarget();
      if (intermediateInternalFormat == 0) {
         ...
         this.downsampleProgram = ShaderProgram.fromResources("assets/hysteria/shaders/blur/blur_fullscreen.vert", "assets/hysteria/shaders/blur/blur_downsample.frag");
         this.upsampleProgram = ShaderProgram.fromResources("assets/hysteria/shaders/blur/blur_fullscreen.vert", "assets/hysteria/shaders/blur/blur_upsample.frag");
         this.smallHProgram = ShaderProgram.fromResources("assets/hysteria/shaders/blur/blur_fullscreen.vert", "assets/hysteria/shaders/blur/blur_small_horizontal.frag");
         this.smallVProgram = ShaderProgram.fromResources("assets/hysteria/shaders/blur/blur_fullscreen.vert", "assets/hysteria/shaders/blur/blur_small_vertical.frag");
   ...
   public void destroy() {
      DownsampleBlur.LevelTarget[] var1 = this.pyramid;
      int var2 = var1.length;

      for(int var3 = 0; var3 < var2; ++var3) {
         DownsampleBlur.LevelTarget level = var1[var3];
   ...
   public int blurFromColorTexture(int sourceTexture, int width, int height, float radiusPx, boolean preserveState) {
      if (sourceTexture != 0 && width > 0 && height > 0) {
         ...
         boolean useSmallKernel = effectiveRadius <= 3.5F;
         int passCount = 0;
         float[] offsets = null;
         if (useSmallKernel) {
         ...
         int var12;
         try {
            TextureUnitGuard unit0 = TextureUnitGuard.capture(0, 3553);
   ...
   private void runSmallRadiusBlur(int sourceTexture, int width, int height, float effectiveRadius) {
      ...
      if (this.smallHTexelSizeLoc >= 0) {
         GL20.glUniform2f(this.smallHTexelSizeLoc, 1.0F / (float)Math.max(1, width), 1.0F / (float)Math.max(1, height));
      }
      ...
      if (this.smallVTexelSizeLoc >= 0) {
         GL20.glUniform2f(this.smallVTexelSizeLoc, 1.0F / (float)Math.max(1, width), 1.0F / (float)Math.max(1, height));
      }
   ...
   private void runDownsampleBlur(int sourceTexture, int width, int height, int passCount, float[] offsets) {
      if (offsets != null && offsets.length == passCount) {
         ...
         int i;
         DownsampleBlur.LevelTarget target;
         for(i = 0; i < passCount; ++i) {
            target = this.pyramid[i];
            this.bindTarget(target);
            if (this.downTexelSizeLoc >= 0) {
               GL20.glUniform2f(this.downTexelSizeLoc, 1.0F / (float)Math.max(1, currentWidth), 1.0F / (float)Math.max(1, currentHeight));
            }
         ...
         for(i = passCount - 2; i >= 0; --i) {
            target = this.pyramid[i];
            this.bindTarget(target);
            if (this.upTexelSizeLoc >= 0) {
               GL20.glUniform2f(this.upTexelSizeLoc, 1.0F / (float)Math.max(1, currentWidth), 1.0F / (float)Math.max(1, currentHeight));
            }
   ...
   private int determinePassCount(float radiusPx, int width, int height) {
      ...
      int desired = Math.max(1, (int)Math.ceil((double)(radiusPx / 8.0F)));
      if (radiusPx <= 16.0F) {
   ...
   private float[] buildOffsets(int passCount, float radiusPx) {
      ...
      float levelScale;
      for(i = 0; i < passCount; ++i) {
         share = (float)i / (float)Math.max(1, passCount - 1);
         levelScale = 1.0F - share * 0.5F;
         weights[i] = levelScale;
         sumWeights += levelScale;
      }

      for(i = 0; i < passCount; ++i) {
         share = weights[i] / sumWeights;
         levelScale = 1.0F / (float)(1 << i);
         offsets[i] = Math.max(0.5F, radiusPx * share * levelScale + 0.5F);
      }
