...
import foure.dev.util.render.postfx.DepthRenderTarget;
import foure.dev.util.render.postfx.DownsampleBlur;
...
public final class GlBackend {
   ...
   public GlBackend() {
      ...
      if (this.ssboSupported || instancedArraysSupported && drawInstancedSupported) {
         String vertexShaderPath = this.ssboSupported ? "assets/hysteria/shaders/shape.vert" : "assets/hysteria/shaders/shape_compat.vert";
         this.shapeProgram = ShaderProgram.fromResources(vertexShaderPath, "assets/hysteria/shaders/shape.frag");
   ...
   private void ensureFullscreenResources() {
      if (this.fullscreenQuadVao == 0) {
         ...
         float[] quad = new float[]{-1.0F, -1.0F, 0.0F, 0.0F, 1.0F, -1.0F, 1.0F, 0.0F, 1.0F, 1.0F, 1.0F, 1.0F, -1.0F, -1.0F, 0.0F, 0.0F, 1.0F, 1.0F, 1.0F, 1.0F, -1.0F, 1.0F, 0.0F, 1.0F};
         GL15.glBufferData(34962, quad, 35044);
         int stride = 16;
         GL20.glEnableVertexAttribArray(0);
   ...
   private ShaderProgram ensureFullscreenProgram() {
      if (this.fullscreenProgram != null) {
         ...
         String vertex = ResourceUtils.readText("assets/hysteria/shaders/blur/blur_fullscreen.vert");
   ...
   public void beginFrame(int width, int height) {
      ...
      GL30.glBindVertexArray(this.vaoDraw);
      GL20.glUniform2f(this.uViewportLoc, (float)width, (float)height);
   ...
   public void flush() {
      if (this.instanceCount > 0) {
         ...
         int trianglesDrawn;
         int slot;
         for(trianglesDrawn = 0; trianglesDrawn < usedSlots; ++trianglesDrawn) {
            GL13.glActiveTexture('蓀' + trianglesDrawn);
            prevBindings[trianglesDrawn] = GL11.glGetInteger(32873);
            slot = (Integer)this.slotToTexture.get(trianglesDrawn);
            GL11.glBindTexture(3553, slot);
         }

         trianglesDrawn = Math.max(0, this.instanceCount) * 2;
         if (trianglesDrawn > 0) {
   ...
   public void setScissorRect(int x, int y, int w, int h, float roundTopLeft, float roundTopRight, float roundBottomRight, float roundBottomLeft) {
      this.clipX = x;
      this.clipY = y;
      this.clipW = w;
      this.clipH = h;
   ...
   private void writeInstanceEx(int type, float x, float y, float w, float h, int colorTL, int colorTR, int colorBR, int colorBL, float roundTL, float roundTR, float roundBR, float roundBL, float thickness, float[] transform, float u0, float v0, float u1, float v1, int texSlot, float startDeg, float arcPct, int extraFlags) {
      if (this.instanceCount >= 4096) {
         ...
         if (type == 2) {
            float normalizedStart = startDeg % 360.0F;
            if (normalizedStart < 0.0F) {
               normalizedStart += 360.0F;
            }

            int encodedStart = Math.max(0, Math.min(255, Math.round(normalizedStart / 360.0F * 255.0F)));
   ...
   private void writeInstanceQuad(int type, float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, int color1, int color2, int color3, int color4, float[] transform) {
      if (this.instanceCount >= 4096) {
         throw new IllegalStateException("Instance capacity exceeded");
      } else {
         int offset = this.instanceCount * 144;
         this.instanceBuffer.position(offset);
         float[] mat = transform != null && transform.length >= 6 ? transform : IDENTITY_TRANSFORM;
         putVertex(this.instanceBuffer, mat, x1, y1);
         putVertex(this.instanceBuffer, mat, x2, y2);
         putVertex(this.instanceBuffer, mat, x3, y3);
         putVertex(this.instanceBuffer, mat, x4, y4);
         int cx = this.clipEnabled ? this.clipX : 0;
         int cy = this.clipEnabled ? this.clipY : 0;
         int cw = this.clipEnabled ? this.clipW : this.viewportWidth;
         int ch = this.clipEnabled ? this.clipH : this.viewportHeight;
         ...
         this.instanceBuffer.putInt(cw);
         this.instanceBuffer.putInt(ch);
         this.instanceBuffer.putFloat(this.clipEnabled ? this.clipRoundTL : 0.0F);
         this.instanceBuffer.putFloat(this.clipEnabled ? this.clipRoundTR : 0.0F);
         this.instanceBuffer.putFloat(this.clipEnabled ? this.clipRoundBR : 0.0F);
         this.instanceBuffer.putFloat(this.clipEnabled ? this.clipRoundBL : 0.0F);
         this.instanceBuffer.putFloat(0.0F);
         this.instanceBuffer.putFloat(0.0F);
         this.instanceBuffer.putFloat(1000000.0F);
         this.instanceBuffer.putFloat(1000000.0F);
         this.instanceBuffer.putInt(packColorRgba(color1));
         this.instanceBuffer.putInt(packColorRgba(color2));
         this.instanceBuffer.putInt(packColorRgba(color3));
         this.instanceBuffer.putInt(packColorRgba(color4));
         this.instanceBuffer.putFloat(0.0F);
         this.instanceBuffer.putFloat(0.0F);
         this.instanceBuffer.putFloat(0.0F);
         this.instanceBuffer.putFloat(0.0F);
         this.instanceBuffer.putFloat(0.0F);
         this.instanceBuffer.putFloat(0.0F);
         this.instanceBuffer.putFloat(1.0F);
         this.instanceBuffer.putFloat(1.0F);
         this.instanceBuffer.putInt(type);
         this.instanceBuffer.putInt(-1);
         this.instanceBuffer.putInt(0);
         this.instanceBuffer.putInt(0);
         ++this.instanceCount;
      }
   ...
   private void writeInstance(int type, float x, float y, float w, float h, int color, float rounding, float thickness, float[] transform, float u0, float v0, float u1, float v1, int texSlot, float startDeg, float arcPct) {
      this.writeInstanceEx(type, x, y, w, h, color, color, color, color, rounding, rounding, rounding, rounding, thickness, transform, u0, v0, u1, v1, texSlot, startDeg, arcPct, 0);
   }

   public void enqueueRect(float x, float y, float w, float h, float roundTopLeft, float roundTopRight, float roundBottomRight, float roundBottomLeft, int color, float[] transform) {
      this.ensureInstanceCapacity();
      this.writeInstanceEx(0, x, y, w, h, color, color, color, color, roundTopLeft, roundTopRight, roundBottomRight, roundBottomLeft, 0.0F, transform, 0.0F, 0.0F, 1.0F, 1.0F, -1, 0.0F, 1.0F, 0);
   }

   public void enqueueRectOutline(float x, float y, float w, float h, float roundTopLeft, float roundTopRight, float roundBottomRight, float roundBottomLeft, int color, float thickness, float[] transform) {
      this.ensureInstanceCapacity();
      this.writeInstanceEx(1, x, y, w, h, color, color, color, color, roundTopLeft, roundTopRight, roundBottomRight, roundBottomLeft, thickness, transform, 0.0F, 0.0F, 1.0F, 1.0F, -1, 0.0F, 1.0F, 0);
   }

   public void enqueueGradient(float x, float y, float w, float h, float roundTopLeft, float roundTopRight, float roundBottomRight, float roundBottomLeft, int c00, int c10, int c11, int c01, float[] transform) {
      this.ensureInstanceCapacity();
      this.writeInstanceEx(0, x, y, w, h, c00, c10, c11, c01, roundTopLeft, roundTopRight, roundBottomRight, roundBottomLeft, 0.0F, transform, 0.0F, 0.0F, 1.0F, 1.0F, -1, 0.0F, 1.0F, 0);
   }

   public void enqueueCircle(float cx, float cy, float radius, float startDeg, float pct, int color, float[] transform) {
      float size = radius * 2.0F;
      this.ensureInstanceCapacity();
      this.writeInstance(2, cx - radius, cy - radius, size, size, color, 0.0F, 0.0F, transform, 0.0F, 0.0F, 1.0F, 1.0F, -1, startDeg, pct);
   }

   public void enqueueQuad(float x1, float y1, float x2, float y2, float x3, float y3, float x4, float y4, int color, float[] transform) {
      this.ensureInstanceCapacity();
      this.writeInstanceQuad(0, x1, y1, x2, y2, x3, y3, x4, y4, color, color, color, color, transform);
   }

   public void drawDropShadowRect(float x, float y, float w, float h, float roundTopLeft, float roundTopRight, float roundBottomRight, float roundBottomLeft, float blurStrength, float spread, int rgbaPremul, float[] transform) {
      if (!(w <= 0.0F) && !(h <= 0.0F)) {
         ...
         if (!(expandedW <= 0.0F) && !(expandedH <= 0.0F)) {
            this.ensureInstanceCapacity();
            this.writeInstanceEx(0, expandedX, expandedY, expandedW, expandedH, rgbaPremul, rgbaPremul, rgbaPremul, rgbaPremul, roundTopLeft, roundTopRight, roundBottomRight, roundBottomLeft, 0.0F, transform, w, h, Math.max(safeBlur, 0.001F), safeSpread, 0, 0.0F, 1.0F, 67108864);
         }
      }
   }

   public void drawTexturedQuad(int texture, float x, float y, float w, float h, float u0, float v0, float u1, float v1, int rgbaPremul, float[] transform) {
      this.ensureInstanceCapacity();
   ...
   public void drawTexturedQuadRounded(int texture, float x, float y, float w, float h, float u0, float v0, float u1, float v1, float rounding, int rgbaPremul, float[] transform) {
      this.ensureInstanceCapacity();
      int slot = this.textureSlotFor(texture);
      this.writeInstance(3, x, y, w, h, rgbaPremul, rounding, 0.0F, transform, u0, v0, u1, v1, slot, 0.0F, 1.0F);
   }

   public void drawRgbaTexturedQuad(int texture, float x, float y, float w, float h, float u0, float v0, float u1, float v1, int rgbaPremul, float[] transform) {
      this.drawRgbaTexturedQuad(texture, x, y, w, h, u0, v0, u1, v1, rgbaPremul, transform, false);
   }

   public void drawRgbaTexturedQuad(int texture, float x, float y, float w, float h, float u0, float v0, float u1, float v1, int rgbaPremul, float[] transform, boolean preservePremultipliedColor) {
      this.ensureInstanceCapacity();
      int slot = this.textureSlotFor(texture);
      int extraFlags = preservePremultipliedColor ? 64 : 0;
      this.writeInstanceEx(3, x, y, w, h, rgbaPremul, rgbaPremul, rgbaPremul, rgbaPremul, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, transform, u0, v0, u1, v1, slot, 0.0F, 1.0F, extraFlags);
   }

   public void drawRgbaTexturedQuadRounded(int texture, float x, float y, float w, float h, float u0, float v0, float u1, float v1, float rounding, int rgbaPremul, float[] transform) {
      this.drawRgbaTexturedQuadRounded(texture, x, y, w, h, u0, v0, u1, v1, rounding, rgbaPremul, transform, false);
   }

   public void drawRgbaTexturedQuadRounded(int texture, float x, float y, float w, float h, float u0, float v0, float u1, float v1, float rounding, int rgbaPremul, float[] transform, boolean preservePremultipliedColor) {
      this.ensureInstanceCapacity();
      int slot = this.textureSlotFor(texture);
      int extraFlags = preservePremultipliedColor ? 64 : 0;
      this.writeInstanceEx(3, x, y, w, h, rgbaPremul, rgbaPremul, rgbaPremul, rgbaPremul, rounding, rounding, rounding, rounding, 1.0F, transform, u0, v0, u1, v1, slot, 0.0F, 1.0F, extraFlags);
   }

   public void drawRgbaOpaqueTexturedQuadRounded(int texture, float x, float y, float w, float h, float u0, float v0, float u1, float v1, float rounding, int rgbaPremul, float[] transform) {
      this.ensureInstanceCapacity();
      this.drawRgbaOpaqueTexturedQuadRounded(texture, x, y, w, h, u0, v0, u1, v1, rounding, rgbaPremul, transform, false);
   }

   public void drawRgbaOpaqueTexturedQuadRounded(int texture, float x, float y, float w, float h, float u0, float v0, float u1, float v1, float rounding, int rgbaPremul, float[] transform, boolean screenSpaceUv) {
      this.ensureInstanceCapacity();
      int slot = this.textureSlotFor(texture);
      int extraFlags = 8;
      if (screenSpaceUv) {
   ...
   public void drawRgbaOpaqueTexturedQuad(int texture, float x, float y, float w, float h, float u0, float v0, float u1, float v1, int rgbaPremul, float[] transform) {
      this.ensureInstanceCapacity();
      int slot = this.textureSlotFor(texture);
      int extraFlags = 8;
      this.writeInstanceEx(3, x, y, w, h, rgbaPremul, rgbaPremul, rgbaPremul, rgbaPremul, 0.0F, 0.0F, 0.0F, 0.0F, 1.0F, transform, u0, v0, u1, v1, slot, 0.0F, 1.0F, extraFlags);
   ...
   public void enqueueMsdfGlyph(int texture, float pxRange, float x, float y, float width, float height, float u0, float v0, float u1, float v1, int rgbaColor, float[] transform) {
      if (texture > 0) {
         this.ensureInstanceCapacity();
         int slot = this.textureSlotFor(texture);
         float clampedRange = pxRange > 0.0F ? pxRange : 0.001F;
         this.writeInstanceEx(3, x, y, width, height, rgbaColor, rgbaColor, rgbaColor, rgbaColor, clampedRange, clampedRange, clampedRange, clampedRange, 0.0F, transform, u0, v0, u1, v1, slot, 0.0F, 1.0F, 16);
      }
   }

   private int textureSlotFor(int texture) {
      Integer slot = (Integer)this.textureToSlot.get(texture);
      if (slot != null) {
         return slot;
      } else {
   ...
   public void uploadAlphaSubImageWithStride(int tex, int x, int y, int w, int h, ByteBuffer data, int sourceRowLength) {
      int prevAlign = GL11.glGetInteger(3317);
      int prevRowLen = GL11.glGetInteger(3314);
      data.order(ByteOrder.nativeOrder());
      GL11.glBindTexture(3553, tex);
      ...
      if (screenW > 0 && screenH > 0) {
         if (Float.isFinite(scaleX) && Float.isFinite(scaleY)) {
            if (!(scaleX <= 0.0F) && !(scaleY <= 0.0F)) {
               ...
               int targetW = Math.max(1, Math.round((float)clampedSrcW * scaleX));
               int targetH = Math.max(1, Math.round((float)clampedSrcH * scaleY));
               if (this.downscaledCaptureTex == 0 || targetW != this.downscaledCaptureW || targetH != this.downscaledCaptureH) {
   ...
   public int captureRegionToTexture(int x, int y, int w, int h, boolean fullscreen) {
      ...
      try {
         ...
         GL11.glDrawBuffer(36064);
         GL30.glBlitFramebuffer(srcX, srcY, srcX + w, srcY + h, 0, 0, w, h, 16384, 9728);
         if (wasScissorEnabled) {
            GL11.glEnable(3089);
         }
      ...
      return fullscreen ? this.captureTex : this.regionCaptureTex;
   }
   ...
   public void prepareScreenBlur(int screenW, int screenH, float radiusPx) {
      if (screenW > 0 && screenH > 0) {
         ...
         if (radiusPx > smallKernelThreshold) {
            actualScaleX = this.screenBlur.minimumRadius();
            actualScaleY = Math.max(radiusPx, actualScaleX);
            float adaptiveScale = smallKernelThreshold / actualScaleY;
         ...
         if (this.downscaledCaptureTex != 0 && this.downscaledCaptureFbo != 0) {
            actualScaleX = (float)this.downscaledCaptureW / (float)Math.max(1, screenW);
            actualScaleY = (float)this.downscaledCaptureH / (float)Math.max(1, screenH);
            ...
            try {
               ...
               GL11.glDrawBuffer(36064);
               GL30.glBlitFramebuffer(0, 0, screenW, screenH, 0, 0, this.downscaledCaptureW, this.downscaledCaptureH, 16384, 9729);
               if (wasScissorEnabled) {
                  GL11.glEnable(3089);
               }
            ...
            float radiusScale = (float)Math.sqrt((double)(Math.max(0.0F, actualScaleX) * Math.max(0.0F, actualScaleY)));
            float scaledRadius = Math.max(0.0F, radiusPx) * radiusScale;
            int blurred = this.screenBlur.blurFromColorTexture(this.downscaledCaptureTex, this.downscaledCaptureW, this.downscaledCaptureH, scaledRadius);
            if (blurred == 0) {
   ...
   public void drawPreparedBlurRounded(float x, float y, float w, float h, float rounding, float alpha, float[] transform) {
      if (this.preparedBlurTex != 0) {
         ...
         float uScale = this.preparedBlurW > 0 ? this.preparedBlurScaleX / (float)this.preparedBlurW : 0.0F;
         float vScale = this.preparedBlurH > 0 ? -this.preparedBlurScaleY / (float)this.preparedBlurH : 0.0F;
         float uOffset = 0.0F;
         float vOffset = this.preparedBlurH > 0 ? 1.0F : 0.0F;
         this.drawRgbaOpaqueTexturedQuadRounded(this.preparedBlurTex, x, y, w, h, uScale, vScale, uOffset, vOffset, rounding, colorPremul, transform, true);
      }
   }

   public void drawPreparedRegionBlurRounded(float x, float y, float w, float h, float rounding, float alpha, float[] transform, int regionX, int regionY, int regionW, int regionH) {
      if (this.preparedRegionBlurTex != 0) {
         if (regionW > 0 && regionH > 0) {
            if (this.preparedRegionBlurW == regionW && this.preparedRegionBlurH == regionH && this.preparedRegionBlurX == regionX && this.preparedRegionBlurY == regionY) {
               this.ensureInstanceCapacity();
               int colorPremul = (int)(Math.max(0.0F, Math.min(1.0F, alpha)) * 255.0F) << 24 | 16777215;
               float invW = regionW > 0 ? 1.0F / (float)regionW : 0.0F;
               float invH = regionH > 0 ? 1.0F / (float)regionH : 0.0F;
               float vScale = regionH > 0 ? -invH : 0.0F;
               float uOffset = (float)(-regionX) * invW;
               float vOffset = regionH > 0 ? 1.0F + (float)regionY * invH : 0.0F;
               this.drawRgbaOpaqueTexturedQuadRounded(this.preparedRegionBlurTex, x, y, w, h, invW, vScale, uOffset, vOffset, rounding, colorPremul, transform, true);
            }
         }
      }
   }
   ...
   public GlBackend.FrameCapture captureFullFrame() {
      ...
      if (client == null) {
         ...
         if (framebuffer == null) {
            ...
            if (!(colorAttachment instanceof GlTexture)) {
               return new GlBackend.FrameCapture(0, 0, 0, 0);
            } else {
               GlTexture glColor = (GlTexture)colorAttachment;
               int sourceColor = glColor.getGlId();
               int sourceDepth = 0;
               GpuTexture depthAttachment = framebuffer.getDepthAttachment();
               if (depthAttachment instanceof GlTexture) {
                  GlTexture glDepth = (GlTexture)depthAttachment;
               ...
               return new GlBackend.FrameCapture(this.fullFrameTarget.colorTex, this.fullFrameTarget.depthTex, width, height);
            }
         }
      }
   }
   ...
   private void installDebugCallback(GLCapabilities caps) {
      if (this.debugCallback == null) {
         this.debugCallback = GLDebugMessageCallback.create((source, type, id, severity, length, message, userParam) -> {
   ...
   private static String severityToString(int severity) {
      ...
      case 37192:
         ...
         break;
      default:
         var10000 = Integer.toString(severity);
      }

      return var10000;
   }

   public static record FrameCapture(int colorTexture, int depthTexture, int width, int height) {
      public FrameCapture(int colorTexture, int depthTexture, int width, int height) {
