...
public class ColonFinder extends Function {
    ...
    private void doScan(int pcx, int pcz, int radius) {
                    ...
                    for (int z = sz; z < sz + 16; z++) {
                        ...
                        for (int y = worldBottom; y < top; y++) {
                            ...
                            for (BlockPattern p : PATTERNS) {
                                ...
                                mn.set(x,y,z-1); ms.set(x,y,z+1); me.set(x+1,y,z); mw.set(x-1,y,z);
        ...
        mc.execute(() -> { renderList = copy; if ((Boolean) notifyEnabled.getValue()) checkColumns(copy); });
    }
    ...
    public void onRender3D(Render3DEvent event) {
        ...
        event.getMatrix().multiply(RotationAxis.POSITIVE_X.rotationDegrees(cam.getPitch()));
        event.getMatrix().multiply(RotationAxis.POSITIVE_Y.rotationDegrees(cam.getYaw() + 180.0F));
        ...
        // render matched blocks
        for (BlockPos pos : renderList) {
            float rx = (float)(pos.getX() - cam.getPos().x);
            float ry = (float)(pos.getY() - cam.getPos().y);
            float rz = (float)(pos.getZ() - cam.getPos().z);
            RenderUtils.renderFilledBox(event.getMatrix(),
                rx, ry, rz, rx + 1, ry + 1, rz + 1, fill);
            RenderUtils.renderBoxOutline(event.getMatrix(),
                rx, ry, rz, rx + 1, ry + 1, rz + 1, outline);
        }
        ...
        if ((Boolean) beamEnabled.getValue()) {
            ...
            for (BlockPos base : beamPosList) {
                float cx = (float)(base.getX() + 0.5 - cam.getPos().x);
                float cz = (float)(base.getZ() + 0.5 - cam.getPos().z);
                float yBot = (float)(base.getY() - cam.getPos().y);
                float yTop = yBot + bh;
                float x1 = cx - half, x2 = cx + half;
                float z1 = cz - half, z2 = cz + half;
                RenderUtils.renderFilledBox(event.getMatrix(), x1, yBot, z1, x2, yTop, z2, beamFill);
                // vertical edges
                RenderUtils.drawLine(event.getMatrix(), x1, yBot, z1, x1, yTop, z1, beamEdge);
                RenderUtils.drawLine(event.getMatrix(), x2, yBot, z1, x2, yTop, z1, beamEdge);
                RenderUtils.drawLine(event.getMatrix(), x2, yBot, z2, x2, yTop, z2, beamEdge);
                RenderUtils.drawLine(event.getMatrix(), x1, yBot, z2, x1, yTop, z2, beamEdge);
            }
        }
