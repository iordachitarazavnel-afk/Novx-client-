...
public class ClickGuiDashboard extends Function {
   ...
   // icon texture cache: iconPath → Identifier
      ...
      while (true) {
         while (var27.hasNext()) {
            ...
            if (!(curY + modH < y + 60.0F) && !(curY > y + h - 10.0F)) {
               ...
               if (!iconPath.isEmpty()) {
                  Identifier texId = getOrLoadIcon(iconPath);
                  if (texId != null) {
                     r.texture(texId, iconX, iconY, iconSize, iconSize);
                  } else {
                     // fallback: colored square if texture not loaded yet
                     r.rect(iconX, iconY, iconSize, iconSize, 4.0F, (new Color(120, 80, 220, 100)).getRGB());
                  }
               }
               // ─────────────────────────────────────────────────────────

               // text — shift right if icon present
               float textOffsetX = iconPath.isEmpty() ? modW / 2.0F : iconSize + 14.0F;
               String textAlign  = iconPath.isEmpty() ? "c" : "l";
               float textX = iconPath.isEmpty() ? curX + textOffsetX : curX + textOffsetX;
               float textY2 = curY + modH / 2.0F + 2.0F;
               int textColor = toggled ? -1 : (new Color(180, 180, 200)).getRGB();
               r.text(FontRegistry.INTER_MEDIUM, textX, textY2, 8.0F, mod.getName(), textColor, textAlign);
