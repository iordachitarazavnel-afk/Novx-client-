...
public class ClickGuiDashboard extends Function {
   ...
   // icon cache: iconPath -> GL texture id (0 = failed/not found)
   ...
   private void drawModuleGrid(Renderer2D r, float x, float y, float w, float h, double rawMx, double rawMy, float scale, float px, float py) {
      ...
      if (searching) {
         ...
         modules = modules.stream().filter((m) ->
            m.getClass().getName().toLowerCase().contains(this.searchString.toLowerCase())
         ).toList();
      } else {
   ...
   // ── Icon loader using Renderer2D.drawRgbaTexture(int, ...) ────────────
   ...
   private void drawSettingsPanel(Renderer2D r, float x, float y, float w, float h, double rawMx, double rawMy, float scale, float px, float py) {
      if (this.selectedModule != null) {
         ...
         r.text(FontRegistry.INTER_MEDIUM, drawX + w / 2.0F, startY + btnH / 2.0F + 2.0F, 7.5F, bindText, -1, "c");
   ...
   public void onPress(EventPress e) {
      if (e.getAction() != 0) {
         ...
         if (this.searchFocused) {
            ...
            else { char c = this.keyToChar(key); if (c != 0) this.searchString = this.searchString + c; }
         } else {
            if (key == 256) this.toggle();
         }
      }
   }
   ...
   public void handleMouseClick(double rawMx, double rawMy, int btn) {
      if (this.isOpen) {
         ...
         if (mx >= (double)this.windowX && mx <= (double)(this.windowX + 140.0F) && my >= (double)this.windowY && my <= (double)(this.windowY + winH)) {
            ...
            for (Category cat : Category.values()) {
               if (cat != Category.THEME && cat != Category.SCRIPT && cat != Category.CONFIG) {
                  if (my >= (double)curY && my <= (double)(curY + 30.0F)) {
                     this.selectedCategory = cat; this.selectedModule = null; this.activeSettings.clear(); return;
                  }
                  curY += 40.0F;
               }
            }
         ...
         if (this.selectedModule != null) {
            ...
            if (mx >= (double)settingsX && mx <= (double)(settingsX + settingsW) && my >= (double)this.windowY && my <= (double)(this.windowY + winH)) {
               ...
               if (mx >= (double)(settingsX + 20.0F) && mx <= (double)(settingsX + settingsW - 20.0F) && my >= (double)bindY && my <= (double)(bindY + 22.0F)) {
                  this.bindingModuleKey = !this.bindingModuleKey; return;
               }
               float curY = bindY + 22.0F + 15.0F;
               for (SettingElement el : this.activeSettings) {
                  if (el.mouseClicked(mx, my, btn, settingsX + 15.0F, curY, settingsW - 30.0F)) return;
                  curY += el.getHeight();
               }
               return;
            }
         ...
         if (mx >= (double)contentX && mx <= (double)(contentX + settingsX) && my >= (double)this.windowY && my <= (double)(this.windowY + winH)) {
            ...
            float startX = contentX + 25.0F;
            float curY = this.windowY + 60.0F - this.scrollOffset;
            float padding = 15.0F;
            float modW = 200.0F;
            float modH = 45.0F;
            float curX = startX;
            for (Function mod : modules) {
               ...
               if (my < (double)(curY + modH) && my > (double)curY && mx < (double)(curX + modW) && mx > (double)curX) {
                  if (btn == 0) { mod.toggle(); }
                     ...
                     if (this.selectedModule == mod) { this.selectedModule = null; this.activeSettings.clear(); }
                     else { this.selectedModule = mod; this.settingsAnim = 0.0F; this.rebuildSettings(); }
                  }
                  return;
               }
               curX += modW + padding;
   ...
   public void handleMouseDrag(double rawMx, double rawMy, int btn, double dx, double dy) {
      if (this.isOpen) {
         ...
         if (this.dragging) {
            ...
            for (SettingElement e : this.activeSettings) {
   ...
   private char keyToChar(int key) {
      ...
      if (key >= 48 && key <= 57) {
         if (shift) {
            if (key == 49) return '!'; if (key == 50) return '@'; if (key == 51) return '#';
      ...
      if (key == 32) return ' ';
      if (key == 45) return (char)(shift ? '_' : '-');
      if (key == 46) return (char)(shift ? '>' : '.');
      if (key == 44) return (char)(shift ? '<' : ',');
      return '\u0000';
   }
