...
public class ClickGuiDashboard extends Function {
   ...
   // ── Icon loader ───────────────────────────────────────────────────────
   // In 1.21.11: NativeImageBackedTexture(Supplier<String>, NativeImage)
   // getGlTexture() returns GpuTexture (not int) — nu mai stocam gl id.
   // Stocam Identifier-ul si lasam TextureManager sa gestioneze.
   // iconCache: iconPath -> true (incarcat) / false (failed)
   private final java.util.Map<String, Boolean> iconLoadedCache = new java.util.HashMap<>();

   private int getOrLoadIcon(String iconPath) {
      // returnam 0 daca nu putem folosi textura ca int GL id in 1.21.11
      // iconCache ramane pentru compatibilitate cu codul existent (Integer)
      ...
      try {
         ...
         // In 1.21.11, getGlTexture() returneaza GpuTexture, nu int.
         // Obtinem GL id din GpuTexture daca este SimpleGlTexture.
         int glId = 0;
         com.mojang.blaze3d.textures.GpuTexture gpuTex = tex.getGlTexture();
         if (gpuTex instanceof com.mojang.blaze3d.platform.NativeImage.SimpleGlTexture sgt) {
            glId = sgt.getId();
         }
