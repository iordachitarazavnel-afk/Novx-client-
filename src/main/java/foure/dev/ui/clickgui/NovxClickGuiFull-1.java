 ...
 * Settings popup: animated, drag-able, auto-positioned for any GUI scale
 */
...
public class NovxClickGuiFull extends Function {
    ...
    // ── Popup animation ───────────────────────────────────────────────────
    private float popupAnim    = 0f;   // 0→1 open, 1→0 close
    private long  popupAnimTime = 0L;
    ...
    private void openPopup(Function mod, double mx, double my) {
        ...
        popupAnim      = 0f;
        popupClosing   = false;
        popupAnimTime  = System.currentTimeMillis();
        bindingKey     = false;

        // Auto-position: always fit on screen
        float sw = mc.getWindow().getScaledWidth();
        float sh = mc.getWindow().getScaledHeight();

        // Try right of module list first
        float tryX = winX + WIN_W + 8f;
        if (tryX + POP_W > sw - 4f) {
            // Not enough space on right → try left of window
            tryX = winX - POP_W - 8f;
        }
        if (tryX < 4f) {
            // Still no room → place inside, right side of content
            tryX = winX + WIN_W - POP_W - 8f;
        }
        // Clamp horizontally
        tryX = Math.max(4f, Math.min(tryX, sw - POP_W - 4f));

        float tryY = (float) my - 30f;
        // Clamp vertically
        tryY = Math.max(4f, Math.min(tryY, sh - POP_H - 4f));
    ...
    public void onRender(RenderEvent event) {
        ...
        // Popup animation
        if (popupModule != null) {
            long elapsed = System.currentTimeMillis() - popupAnimTime;
            float speed  = 8f;
            if (!popupClosing) {
                popupAnim = Math.min(1f, popupAnim + dt * speed);
            } else {
                popupAnim = Math.max(0f, popupAnim - dt * speed);
                if (popupAnim <= 0f) { closePopup(); }
            }
            if (popupModule != null) drawPopup(r, popupAnim);
        }
    ...
    private void drawModules(Renderer2D r, float x, float y, float w, float h) {
        ...
        for (Function mod : mods) {
            ...
            if (tog) {
                r.rect(mx2, curY, mw, mH, 5f, new Color(acc.getRed(),acc.getGreen(),acc.getBlue(),30).getRGB());
                r.rectOutline(mx2, curY, mw, mH, 5f, new Color(acc.getRed(),acc.getGreen(),acc.getBlue(),100).getRGB(), 1f);
            } else {
                r.rect(mx2, curY, mw, mH, 5f, new Color(255,255,255, sel?12:6).getRGB());
                if (sel) r.rectOutline(mx2, curY, mw, mH, 5f, new Color(acc.getRed(),acc.getGreen(),acc.getBlue(),40).getRGB(), 1f);
            }
            ...
            r.text(FontRegistry.INTER_MEDIUM, mx2+20f, curY+mH/2f+2f, 8f, mod.getName(), tc, "l");
    ...
    private void drawThemes(Renderer2D r, float x, float y, float w, float h) {
        ...
        for (int i=0; i<THEMES.length; i++) {
            ...
            if (active) {
                r.text(FontRegistry.INTER_MEDIUM,ix+iw-55f,curY+iH/2f+2f,7.5f,"✓ Active",new Color(t.accent.getRed(),t.accent.getGreen(),t.accent.getBlue(),220).getRGB(),"l");
            } else {
                ...
                r.text(FontRegistry.INTER_MEDIUM,bX+23f,bY2+11f,7.5f,"Apply",new Color(t.accent.getRed(),t.accent.getGreen(),t.accent.getBlue(),220).getRGB(),"c");
            }
            curY+=iH+gap;
        }
        r.popClipRect();
    ...
    private void drawFriends(Renderer2D r, float x, float y, float w, float h) {
        ...
        List<String> filtered=friends.stream().filter(f->friendSearch.isEmpty()||f.toLowerCase().contains(friendSearch.toLowerCase())).toList();
        r.pushClipRect((int)(x+8f),(int)lY,(int)(w-16f),(int)lH);
        if (filtered.isEmpty()){
            r.text(FontRegistry.INTER_MEDIUM,x+w/2f,lY+lH/2f,8f,friends.isEmpty()?"No friends added yet":"No results",new Color(70,70,85).getRGB(),"c");
        } else {
    ...
    private void drawPopup(Renderer2D r, float anim) {
        ...
        // Animate: scale from center of popup + fade
        float cx = popupX + POP_W / 2f;
        float cy = popupY + POP_H / 2f;
        float sc = 0.88f + 0.12f * eased;
        r.pushScale(sc, cx, cy);
        r.pushAlpha(eased);
        ...
        r.text(FontRegistry.INTER_MEDIUM, popupX+POP_W/2f, bY+12f, 7.5f,
            bindingKey ? "Listening... (ESC = remove)" : "Bind: " + KeyNameUtil.getKeyName(popupModule.getKey()), -1, "c");
        ...
        float totalSettH = popupElements.stream().mapToFloat(SettingElement::getHeight).sum();
        if (totalSettH > sH) {
            float maxS = totalSettH - sH;
            popupScroll = Math.max(0, Math.min(popupScroll, maxS));
            float tH = Math.max(16f, sH / totalSettH * sH);
            float tY = sY + (popupScroll / maxS) * (sH - tH);
            r.rect(popupX+POP_W-5f, sY, 3f, sH, 1.5f, new Color(255,255,255,12).getRGB());
            r.rect(popupX+POP_W-5f, tY, 3f, tH, 1.5f, new Color(acc.getRed(),acc.getGreen(),acc.getBlue(),90).getRGB());
        }
    ...
    public void handleMouseClick(double rawMx, double rawMy, int btn) {
        ...
        if (mx>=cX&&mx<=winX+WIN_W&&my>=winY&&my<=winY+HEADER_H&&btn==0&&popupModule==null) {
        ...
        if (mx>=winX&&mx<=winX+SIDE_W) {
            ...
            for (int i=0;i<4;i++) {
                if (my>=bNavY&&my<=bNavY+22f) { selTab=tabIdxMap[i]; closePopup(); return; }
                bNavY+=26f;
            }
            float cy=winY+HEADER_H+6f;
            for (Category cat:Category.values()) {
                if (cat==Category.THEME||cat==Category.SCRIPT||cat==Category.CONFIG) continue;
                if (my>=cy&&my<=cy+25f) { selCategory=cat; selTab=0; modScroll=0; return; }
                cy+=27f;
            }
            return;
        }
        ...
        if (selTab==0) {
                ...
                for (Function mod:mods) {
                    ...
                    if (my>=curY&&my<=curY+mH&&mx>=mx2&&mx<=mx2+mw2) {
                        if (btn==0) mod.toggle();
                            ...
                            if (popupModule==mod) { closePopup(); }
                            else { openPopup(mod, mx, my); }
                        }
                        return;
                    }
                    curY+=mH+gap;
            ...
            tY=handleToggleClick(mx,my,contentX,tY,cW,btn);
            tY=handleToggleClick(mx,my,contentX,tY,cW,btn);
            tY+=10f;
    ...
    private void handleConfigClick(double mx, double my, float x, float w, int btn) {
        ...
        if (cfgTab==0) {
            ...
            float sY=inY+28f;
            if (mx>=x+10f&&mx<=x+w-10f&&my>=sY&&my<=sY+22f&&!cfgName.isEmpty()){savedConfigs.add(cfgName);cfgName="";cfgFocus=false;return;}
        }
    }
    private void handleFriendsClick(double mx, double my, float x, float w, int btn) {
        ...
        for (String fr:friends){
            if (my>=curY&&my<=curY+22f&&mx>=x+w-26f&&mx<=x+w-10f){FriendManager.getInstance().removeFriend(fr);return;}
            curY+=26f;
        }
    ...
    public void handleMouseScroll(double amount) {
        if (!isOpen) return;
        // Scroll popup settings if mouse over popup
        if (popupModule != null) {
            float sY=popupY+POP_HDR+48f, sH=POP_H-(sY-popupY)-6f;
            float totalSettH=popupElements.stream().mapToFloat(SettingElement::getHeight).sum();
            if (totalSettH > sH) { popupScroll-=(float)amount*16f; return; }
        }
    ...
    public void onPress(EventPress e) {
        ...
        // ── Keybind capture con support remove (ESC/Delete = rimuovi) ────
        if (bindingKey&&popupModule!=null) {
            if (key==256||key==261) { popupModule.setKey(-1); }
            else                    { popupModule.setKey(key); }
